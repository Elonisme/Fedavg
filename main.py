...
from models.Update import ClientPoint
...
if __name__ == '__main__':
    ...
    if args['model'] == 'cnn' and args['dataset'] == 'cifar':
        ...
        for x in img_size:
            len_in *= x
        glob_model = MLP(dim_in=len_in, dim_hidden=200, dim_out=args['num_classes']).to(args['device'])
    else:
    ...
    for iter in range(args['epochs']):
        ...
        for idx in chosen_user:
            ...
            client_model = ClientPoint(args=args, dataset=dataset_train, idxs=dict_users[idx])
            w, loss = client_model.train(net=copy.deepcopy(glob_model).to(args['device']))
            if args['all_clients']:
    ...
    plt.savefig('./save/fed_{}_{}_{}_C{}_iid{}.png'.format(args['dataset'], args['model'], args['epochs'], args['frac'],
                                                           args['iid']))
